...
class TestPlayRoute:
    ...
    def test_play_when_game_over_redirects(self, client):
        ...
        with client.session_transaction() as sess:
            sess['game_type'] = 'a'
            sess['tuomari'] = {'ekan_pisteet': 3, 'tokan_pisteet': 2, 'tasapelit': 0}
            sess['game_over'] = True
...
class TestMakeMove:
    ...
    def test_make_move_invalid_first_move(self, client):
        ...
        response = client.post(
            '/move',
            data={'ekan_siirto': 'x', 'tokan_siirto': 'k'},
            follow_redirects=False
        )
        assert response.status_code == 302
        assert '/play' in response.location
    ...
    def test_make_move_pvp_invalid_second_move(self, client):
        ...
        response = client.post(
            '/move',
            data={'ekan_siirto': 'k', 'tokan_siirto': 'invalid'},
            follow_redirects=False
        )
        assert response.status_code == 302
        assert '/play' in response.location
    ...
    def test_make_move_game_ends_at_3_wins_player1(self, client):
        """Test game ends when player 1 reaches 3 wins"""
        with client.session_transaction() as sess:
            sess['game_type'] = 'a'
            sess['tuomari'] = {'ekan_pisteet': 2, 'tokan_pisteet': 1, 'tasapelit': 0}
            sess['round'] = 4
            sess['game_over'] = False
    ...
    def test_make_move_game_ends_at_3_wins_player2(self, client):
        """Test game ends when player 2 reaches 3 wins"""
        with client.session_transaction() as sess:
            sess['game_type'] = 'a'
            sess['tuomari'] = {'ekan_pisteet': 1, 'tokan_pisteet': 2, 'tasapelit': 0}
            sess['round'] = 4
            sess['game_over'] = False
...
class TestGameOver:
    ...
    def test_game_over_player1_wins(self, client):
        ...
        with client.session_transaction() as sess:
            sess['game_type'] = 'a'
            sess['tuomari'] = {'ekan_pisteet': 3, 'tokan_pisteet': 1, 'tasapelit': 2}
    ...
    def test_game_over_player2_wins_pvp(self, client):
        ...
        with client.session_transaction() as sess:
            sess['game_type'] = 'a'
            sess['tuomari'] = {'ekan_pisteet': 1, 'tokan_pisteet': 3, 'tasapelit': 1}
...
class TestAILogic:
    ...
    def test_simple_ai_cycles_through_moves(self, client):
        ...
        # Second state should become 2
        client.post('/move', data={'ekan_siirto': 'k'})

        with client.session_transaction() as sess:
            assert sess['ai_siirto'] == 2

        # Third state should become 0
        client.post('/move', data={'ekan_siirto': 'k'})
...
class TestFullGameFlow:
    ...
    def test_complete_pvp_game_flow(self, client):
        ...
        # Play 3 rounds where player 1 always wins -> should end at 3 wins
        for _ in range(3):
    ...
    def test_complete_ai_game_flow(self, client):
        ...
        # Play enough rounds until someone reaches 3 wins
        for _ in range(10):
            with client.session_transaction() as sess:
    ...
    def test_mixed_results_game(self, client):
        ...
        client.post('/move', data={'ekan_siirto': 'k', 'tokan_siirto': 'k'})  # tie
        client.post('/move', data={'ekan_siirto': 'k', 'tokan_siirto': 's'})  # p1 wins
        client.post('/move', data={'ekan_siirto': 's', 'tokan_siirto': 'k'})  # p2 wins
